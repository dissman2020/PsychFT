...
class CrossEntropyKLDLoss(nn.Module):
    def __init__(self, alpha=0.98, ignore_index=-100, kl_reduction='batchmean'):
        super().__init__()
        self.alpha = alpha
    ...
    def forward(self, logits, labels, teacher_logits):
        ...
        if mask.sum() > 0:
            student = F.log_softmax(logits_flat[mask], dim=-1)
            teacher = F.softmax(teacher_logits, dim=-1)
            min_len = min(student.size(0), teacher.size(0))
            if student.size(0) != teacher.size(0):
                print(f"[WARN] KL row mismatch: student={student.size(0)} vs teacher={teacher.size(0)} → clipping to {min_len}")
            student = student[:min_len]
            teacher = teacher[:min_len]
            kld_loss = self.kld(student, teacher)
        else:
