...
class SoftAdaptLoss(nn.Module):
    def __init__(self, model, s_a_beta, ignore_index=-100, kl_reduction='batchmean'):
        super().__init__()
        self.model = model
        self.s_a_beta = s_a_beta
        ...
        self.current_weights = torch.tensor([0.8, 0.1, 0.1])
        self.step_counter = 0
    ...
    def forward(self,
        ...
        self.loss_history["kld"].append(kld_loss.detach())
        ...
        if self.step_counter % Update_Step == 0 and all(len(v) == Update_Step for v in self.loss_history.values()):
            ...
            # 如果尚未初始化 prev_avg_losses，则赋初值并跳过权重更新
            if any(v is None for v in self.prev_avg_losses.values()):
                self.prev_avg_losses = current_avg
            # 计算s_k并更新权重
            else:
                s_vec = []
                for k in ["ce", "kld", "dws"]:
                    prev = self.prev_avg_losses[k]
                    curr = current_avg[k]
                    s_k = (curr - prev).item()
                    s_vec.append(s_k)
                    self.prev_avg_losses[k] = curr

                s_tensor = torch.tensor(s_vec, dtype=torch.float32, device=logits.device)
                base_weights = self.current_weights.to(logits.device)
                soft_weights = torch.softmax(self.s_a_beta * s_tensor, dim=0)
                uniform = torch.full_like(soft_weights, 1 / 3)

                # 计算偏差
                adjustment = MIX_RATIO * (soft_weights - uniform)

                # 更新权重
                new_weights = base_weights + adjustment

                # 防止负数，然后归一化
                new_weights = torch.clamp(new_weights, min=1e-6)
                self.current_weights = new_weights / new_weights.sum()


            # 计算 total_loss_avg 并记录日志
            alpha = self.current_weights.to(logits.device)
            total_loss_avg = (alpha[0] * current_avg["ce"] + alpha[1] * current_avg["kld"] + alpha[2] * current_avg["dws"])

            self._log_losses(current_avg["ce"], current_avg["kld"], current_avg["dws"], total_loss_avg, step, alpha)

        # 使用当前权重计算总损失
        alpha = self.current_weights.to(logits.device)
        total_loss = alpha[0] * ce_loss + alpha[1] * kld_loss + alpha[2] * dws_loss
    ...
    def _log_losses(self, ce_loss, kld_loss, dws_loss, total_loss, step, alpha):
        if accelerator.is_main_process:
            print(f"[LOSS] CE = {ce_loss.item():.4f}, KL = {kld_loss.item():.4f}, DWS = {dws_loss.item():.4f}, total = {total_loss.item():.4f}")
            print(f"[WEIGHTS] CE = {alpha[0]:.4f}, KL = {alpha[1]:.4f}, DWS = {alpha[2]:.4f}")
            swanlab.log({
                "Step CE Loss": ce_loss.item(),
                "Step KLD Loss": kld_loss.item(),
                "Step DWS Loss": dws_loss.item(),
                "Step CE Weight": alpha[0].item(),
                "Step KLD Weight": alpha[1].item(),
                "Step DWS Weight": alpha[2].item(),
...
class CustomSFTTrainer(SFTTrainer):
    def __init__(self, *args, **kwargs):
        ...
        self.loss_fn = SoftAdaptLoss(self.model, s_a_beta=1.0)
