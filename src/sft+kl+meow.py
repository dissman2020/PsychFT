...
class SoftAdaptLoss(nn.Module):
    def __init__(self, model, s_a_beta, ignore_index=-100, kl_reduction='batchmean'):
        ...
        self.current_weights = torch.tensor([1 / 2, 1 / 2])
        self.step_counter = 0
    ...
    def forward(self,
        ...
        if self.step_counter % Update_Step == 0 and all(len(v) == Update_Step for v in self.loss_history.values()):
            ...
            for k in ["ce", "kld"]:
                ...
                curr = current_avg[k]
                s = 0.0 if prev is None else (curr - prev).item()
                s_k = GAMMA * (prev.item() if prev is not None else 0.0) + (1 - GAMMA) * s
                s_vec.append(s_k)
                self.prev_avg_losses[k] = curr

            s_tensor = torch.tensor(s_vec, dtype=torch.float32, device=logits.device)
            self.current_weights = torch.softmax(self.s_a_beta * s_tensor, dim=0)

            # 计算 total_loss_avg 并记录日志
            alpha = self.current_weights.to(logits.device)
            total_loss_avg = (alpha[0] * current_avg["ce"] + alpha[1] * current_avg["kld"])

            self._log_losses(current_avg["ce"], current_avg["kld"], total_loss_avg, step, alpha)

        # 使用当前权重计算总损失
        alpha = self.current_weights.to(logits.device)
        total_loss = alpha[0] * ce_loss + alpha[1] * kld_loss
    ...
    def _log_losses(self, ce_loss, kld_loss, total_loss, step, alpha):
        if accelerator.is_main_process:
            print(f"[LOSS] CE = {ce_loss.item():.4f}, KL = {kld_loss.item():.4f}, total = {total_loss.item():.4f}")
            print(f"[WEIGHTS] CE = {alpha[0]:.4f}, KL = {alpha[1]:.4f}")
            swanlab.log({
